...
def phonon_qe_1(batch_mode):
    ...
    dynamics = CEoM(
        ...
        sub_steps=None,                  # number of sub steps
        solver='qe',                     # choice of integrator
        save_file='exact_09.pic',        # if not None, generated data will be saved to this file
    ...
    # Set up a classical initial state
    # Set up an initial state: coherent state
    dynamics.coherent(
        displace=[1.0 if i == hamilton.n_site//2 else 0.0 for i in range(hamilton.n_site)]
    )
    ...
    # Interactive mode: Setup animated visualization
        ...
        graphics = VisualCEoM(
            ...
            plot_style={'xlimits': (-1.5, 1.5), 'ylimits': (-0.002, 0.002)}  # intended for PhaseSpace only
        ).create()
